...
/**
 * Ein einziger, durchgaengiger Demolauf mit sichtbarem Mauszeiger.
 *
 * Grundregel dieses Videos: Vitalwerte werden NICHT eingetippt. Jeder Wert
 * entsteht aus der Position des Mauszeigers in der Grafik – durch Klicken, im
 * Kontrollzeit-Modus oder durch Ziehen der Griffe. Der Test liest dazu die
 * Koordinatenanzeige der Anwendung (Zeit und Y-Wert unter dem Zeiger) und
 * behauptet anschliessend, dass genau der angezeigte Wert gespeichert wurde.
 *
 * Die Browser-Uhr ist auf den 04.08.2026 fixiert; die Zeitachse beginnt bei
 * 10:00:00.
 */

const CASE_KEY = "sikant-anesthesia-demo-case:v1";
const PATIENT_KEY = "sikant-anesthesia-demo.patient-base-data.v1";
const CROSSHAIR = '[data-testid="crosshair-coordinate"]';

// Fester Demotag: 04.08.2026 (Monat ist 0-basiert).
const DEMO_DAY = { year: 2026, month: 7, day: 4 } as const;
const DEMO_DATE_DE = "04.08.2026";
...
function ts(clock: string): number {
 ...
 */
async function travelTo(page: Page, clock: string) {
...
interface CaseSnapshot {
  ...
  infusions: Array<{ id: string; name: string; startedAt: number; endedAt: number | null }>;
  events: Array<{ id: string; eventType: string; time: number }>;
}

async function caseData(page: Page): Promise<CaseSnapshot> {
  return page.evaluate((key) => JSON.parse(localStorage.getItem(key) ?? "{}"), CASE_KEY);
}

async function measurementAt(page: Page, kind: string, time: number) {
  await expect
...
async function newestMeasurement(page: Page, kind: string, previousCount: number) {
  await expect
...
async function countOf(page: Page, kind: string) {
...
function parseCrosshair(text: string): { clock: string; value: number } {
 ...
 */
async function crosshairState(page: Page) {
...
async function probe(page: Page, x: number, y: number): Promise<{ clock: string; value: number }> {
 ...
 */
async function settleCheckpointCrosshair(page: Page, y: number) {
  await expect
    .poll(async () => {
...
async function plotBoxes(page: Page, kind: string) {
  ...
  // Kalibriert wird links neben der Jetzt-Linie: dort ist jede Zeit gueltig und
  // die Jetzt-Fangzone (6 px) sicher ausserhalb.
  const probeX = Math.max(areaBox!.x + 8, nowBox!.x + nowBox!.width / 2 - 34);
 ...
 */
async function calibrateBand(page: Page, kind: string) {
...
async function yForValue(page: Page, kind: string, target: number) {
...
async function readDraftValue(page: Page, kind: string): Promise<number> {
...
async function clickNibpMean(page: Page, targetMean: number, options: { x?: number; clock?: string } = {}) {
 ...
 */
async function dragNibpHandle(page: Page, id: string, part: "systolic" | "diastolic", target: number) {
  const valueOf = async () => {
...
async function dragScalarPoint(page: Page, kind: string, id: string, target: number) {
 ...
 */
async function pressValueAtCheckpoint(page: Page, kind: string, target: number) {
  const { band, probeX } = await plotBoxes(page, kind);
  ...
  await page.mouse.down();
  const a = await settleCheckpointCrosshair(page, yA);
  await page.mouse.move(probeX, yB, { steps: 10 });
  const b = await settleCheckpointCrosshair(page, yB);
  ...
  const shown = await settleCheckpointCrosshair(page, targetY);
  await page.mouse.up();
...
async function setTimePicker(page: Page, testId: string, clock: string) {
...
async function selectBasisOption(page: Page, field: string, label: string) {
...
async function documentEvent(page: Page, eventType: string, clock: string) {
...
test("Narkoseprotokoll-Demo: Werte per Maus dokumentieren, korrigieren und abschliessen", async ({ page }) => {
  ...
  await test.step("Speichern und Schließen: Kontrollseite vor dem Sichern durchsehen", async () => {
    ...
    await review.waitForURL("**/abschluss");
